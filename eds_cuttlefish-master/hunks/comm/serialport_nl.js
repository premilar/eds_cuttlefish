...
// BUT: before other things, really want this to successfully reset
// *reliably* - killing the whole downstream process
...
//  this.inputs.push(dtin)
  ...
  let dtout = new Output('string', 'data', this)
  ...
  let usbPidSelect = new State('string', 'usb product id', '6001')
  ...
  // hack-alert: downstream code uses hard coded vals, rip, apologies
  this.states.push(pipeStatusMessage, pipeRetryButton, serialStatusMessage) //portSelect, usbPidSelect,
  ...
  let startWsConnection = () => {
    ...
    jQuery.get(`spawnProcess/vfpts_nl.js?args=${portSelect.value},${usbPidSelect.value}`, (data) => {
      if (data.startup) {
    ...
    if(typeof msg.data === 'string'){
      ...
      if(msg.data instanceof Blob){
        ...
        msg.data.text().then((str) =>{
          //console.log('blob is', str)
          outbuffer.push(str)
        }).catch((err)=>{
          console.error('err blob -> text in serialport')
        })
      } else {
        // bad error state
